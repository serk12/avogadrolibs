...
{
  assert(index < size());
  uniqueGroup.removeConection(index);
...
void Graph::removeEdge(size_t a, size_t b)
{
  ...
  uniqueGroup.removeConection(a, neighborsA, b, neighborsB);
...
{
    return uniqueGroup.getAllGroups();
}

std::set<size_t> Graph::connectedComponent(size_t index) const
{
    size_t group = uniqueGroup.getGroup(index);
    return uniqueGroup.getElements(group);
}
